...
public class BrokerController {
    ...
    public boolean initialize() throws CloneNotSupportedException {
        ...
        if (result) {
            ...
            // 当前 broker 配置 了 namesrv 地址
            if (this.brokerConfig.getNamesrvAddr() != null) {
                // 一般都会配置
                this.brokerOuterAPI.updateNameServerAddressList(this.brokerConfig.getNamesrvAddr());
                log.info("Set user specified name server address: {}", this.brokerConfig.getNamesrvAddr());
            } else if (this.brokerConfig.isFetchNamesrvAddrByAddressServer() /* 默认是关闭的 */) {
                this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {

                    @Override
                    public void run() {
                        try {
                            String serverAddr = BrokerController.this.brokerOuterAPI.fetchNameServerAddr();
            ...
            if (TlsSystemConfig.tlsMode != TlsMode.DISABLED) {
                ...
                try {
                    fileWatchService = new FileWatchService(new String[] { TlsSystemConfig.tlsServerCertPath, TlsSystemConfig.tlsServerKeyPath, TlsSystemConfig.tlsServerTrustCertPath },
    ...
    private void initialTransaction() {
        this.transactionalMessageService = ServiceProvider.loadClass(ServiceProvider.TRANSACTION_SERVICE_ID/*META-INF/service/org.apache.rocketmq.broker.transaction.TransactionalMessageService*/, TransactionalMessageService.class);
    ...
    private void doRegisterBrokerAll(boolean checkOrderConfig, boolean oneway, TopicConfigSerializeWrapper topicConfigWrapper) {
        ...
        if (registerBrokerResultList.size() > 0) {
            ...
            if (registerBrokerResult != null) {
                if (this.updateMasterHAServerAddrPeriodically/*false*/ && registerBrokerResult.getHaServerAddr() != null) {
